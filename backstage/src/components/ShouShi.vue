...
export default {
  ...
  methods: {
    fond() {
      //查询触发函数
      this.$http //发起ajax请求
        .get("http://localhost:9999/shoushifond", {
          params: {
    ...
    submitForm(formName) {
      ...
      this.$http //发起ajax请求
        .get("http://localhost:9999/shoushiadd", {
          params: {
    ...
    send(index) {
      //页面加载完时触发函数
      this.$http //发起ajax请求
        .get("http://localhost:9999/shoushi", {
         params:{
             index:index
         } 
        })
    ...
    delAll: function() {
      ...
      this.$confirm("确认删除这些记录吗?", "提示", {
        ...
        .then(() => {
          //弹框"确定"按钮触发函数
          this.$http //发起ajax请求
            .get("http://localhost:9999/shoushidelall", {
              params: {
      ...
      if (this.sels.length != 0) {
        ...
        this.$confirm("确认删除该记录吗?", "提示", {
          ...
          .then(() => {
            //弹框确定按钮触发
            this.$http //发起ajax请求
              .get("http://localhost:9999/shoushidel", {
                params: {
